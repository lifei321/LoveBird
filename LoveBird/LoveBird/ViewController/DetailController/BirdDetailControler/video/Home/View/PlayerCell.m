...
- (void)setViedioModel:(BirdDetailVedioModel *)viedioModel {

    _viedioModel = viedioModel;
    [self.IMGView sd_setImageWithURL:[NSURL URLWithString:viedioModel.video_imgUrl] placeholderImage:nil];
//    __block UIImage *placeholderImage = [UIImage imageNamed:@"placeholder"];
//    [[SDWebImageManager sharedManager] cachedImageExistsForURL:[NSURL URLWithString:viedioModel.pictureUrl] completion:^(BOOL isInCache) {
//        if (isInCache) {
//            //本地存在图片,替换占位图片
